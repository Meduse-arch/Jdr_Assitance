...
const Fiche = ({ playerData, auth, sessionId, onRefresh }) => {
  ...
  const [showOthersList, setShowOthersList] = useState(false); // NOUVEAU
  ...
  // NOUVEAU : Slot groupé pour "Autres" (10 slots)
  const OthersGroupSlot = () => {
    let count = 0;
    for (let i = 1; i <= 10; i++) { if (eq[`autre${i}`]) count++; }
    
    return (
      <div onClick={() => setShowOthersList(true)} className={`flex items-center gap-3 p-3 rounded-lg border w-full h-16 cursor-pointer transition-all mt-4 ${count > 0 ? 'bg-[#1a1a1a] border-yellow-500/50 hover:bg-[#252525]' : 'bg-black/20 border-gray-800 border-dashed'}`}>
        <span className="text-2xl opacity-80 w-8 text-center">🎒</span>
        <div className="flex flex-col">
            <span className="text-[8px] uppercase font-bold text-gray-500">AUTRES</span>
            <span className={`text-xs font-bold ${count > 0 ? 'text-yellow-300' : 'text-gray-700 italic'}`}>{count} / 10 Équipés</span>
        </div>
      </div>
    );
  };
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={() => setSelectedEquip(null)}>
        <div className="bg-[#1a1a1a] border border-gray-600 w-full max-w-sm rounded-xl overflow-hidden shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-3">
                ...
                {selectedEquip.modifiers && (
                    <div className="grid grid-cols-2 gap-2">
                        {Object.entries(selectedEquip.modifiers).map(([k, v]) => (
                            <div key={k} className="bg-[#111] border border-gray-700 px-2 py-1 rounded flex justify-between">
                                <span className="text-xs text-gray-400 capitalize">{k === 'modEffect' ? 'Sort' : k}</span>
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={() => setShowJewelryList(false)}>
            <div className="bg-[#1a1a1a] border border-purple-500/30 w-full max-w-sm rounded-xl overflow-hidden shadow-2xl relative" onClick={e => e.stopPropagation()}>
                <div className="bg-[#111] p-4 flex justify-between items-center border-b border-gray-700">
                    <h3 className="text-lg font-bold text-white">💍 Vos Bijoux</h3>
                    <button onClick={() => setShowJewelryList(false)} className="text-gray-500 hover:text-white text-xl">✕</button>
                </div>
                <div className="p-4 space-y-3 max-h-[60vh] overflow-y-auto custom-scrollbar">
                    {[{s:'Slot 1', i:eq.bijou1}, {s:'Slot 2', i:eq.bijou2}, {s:'Slot 3', i:eq.bijou3}, {s:'Slot 4', i:eq.bijou4}].map((b, idx) => (
                        <div key={idx} className="flex items-center gap-3">
                             <span className="text-[10px] uppercase font-bold text-gray-500 w-10">{b.s}</span>
                             {b.i ? (
                                 <button 
                                    onClick={() => { setShowJewelryList(false); setSelectedEquip(b.i); }}
                                    className="flex-1 bg-[#151515] border border-purple-900/50 hover:bg-[#222] hover:border-purple-500/50 rounded p-2 flex items-center justify-between group transition-all"
                                 >
                                     <div className="flex items-center gap-2">
                                         <span className="text-lg">💍</span>
                                         <span className="text-sm font-bold text-purple-200 group-hover:text-white">{b.i.name}</span>
                                     </div>
                                     <span className="text-xs text-gray-600 group-hover:text-purple-400">►</span>
                                 </button>
                             ) : (
                                 <div className="flex-1 bg-black/20 border border-gray-800 border-dashed rounded p-2 text-center">
                                     <span className="text-xs text-gray-600 italic">Vide</span>
                                 </div>
                             )}
                        </div>
                    ))}
  ...
  // NOUVEAU : Popup pour les objets "Autres" (10 slots)
  const OthersListPopup = () => {
    if (!showOthersList) return null;
    
    const slots = [];
    for (let i = 1; i <= 10; i++) slots.push({ s: `Slot ${i}`, i: eq[`autre${i}`] });

    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={() => setShowOthersList(false)}>
            <div className="bg-[#1a1a1a] border border-yellow-500/30 w-full max-w-sm rounded-xl overflow-hidden shadow-2xl relative flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
                <div className="bg-[#111] p-4 flex justify-between items-center border-b border-gray-700">
                    <h3 className="text-lg font-bold text-white">🎒 Autres Équipements</h3>
                    <button onClick={() => setShowOthersList(false)} className="text-gray-500 hover:text-white text-xl">✕</button>
                </div>
                <div className="p-4 space-y-3 overflow-y-auto custom-scrollbar flex-1">
                    {slots.map((b, idx) => (
                        <div key={idx} className="flex items-center gap-3">
                             ...
                             {b.i ? (
                                 <button 
                                    onClick={() => { setShowOthersList(false); setSelectedEquip(b.i); }}
                                    className="flex-1 bg-[#151515] border border-yellow-900/50 hover:bg-[#222] hover:border-yellow-500/50 rounded p-2 flex items-center justify-between group transition-all"
                                 >
                                     <div className="flex items-center gap-2">
                                         <span className="text-lg">📦</span>
                                         <span className="text-sm font-bold text-yellow-200 group-hover:text-white">{b.i.name}</span>
                                     </div>
                                     <span className="text-xs text-gray-600 group-hover:text-yellow-400">►</span>
                                 </button>
                             ) : (
                                 <div className="flex-1 bg-black/20 border border-gray-800 border-dashed rounded p-2 text-center">
                                     <span className="text-xs text-gray-600 italic">Vide</span>
  ...
  return (
    <div className="animate-fade-in relative pb-10">
      ...
      {/* SECTION DU HAUT : STATS PURES + AVATAR */}
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-stretch">
        ...
        {/* COLONNE CENTRALE AVEC BONUS ET "AUTRES" */}
        <div className="flex-1 flex flex-col gap-4">
            <div className="bg-black/20 rounded-xl border border-gray-800/50 p-4 flex flex-col items-center justify-center relative min-h-[200px]">
                    ...
                    <div className="space-y-2 w-full mt-4">
                        {Object.entries(totalBonuses).map(([stat, val]) => (
                            <div key={stat} className="flex justify-between items-center border-b border-gray-800 pb-1 last:border-0">
                                <span className="text-xs text-gray-400 capitalize">{stat === 'modEffect' ? 'Sort' : stat}</span>
