...
app.post("/api/sessions/create", async (req, res) => {
  ...
  const db = await readDB();
  if (db.sessions[session_id] && !force) return res.status(409).json({ error: "Existe déjà", exists: true });
...
app.post("/api/player/init", async (req, res) => {
  ...
  const hpMax = Math.max(0, constitution * 4);
  const manaMax = Math.max(0, intelligence * 20);
